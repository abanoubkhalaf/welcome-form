...
export default function ChurchForm() {
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      ...
      toast.success(`تم رفع الاستمارة بنجاح كملف PDF! 📁 المجلد: ${result.folder}`);
    ...
    <div className="w-full flex justify-center pb-20">
      ...
      >
        ...
        <form className="space-y-8 text-sm sm:text-base font-bold text-black" onSubmit={(e) => e.preventDefault()} id="churchForm">
          <fieldset disabled={isExporting} className="space-y-8 disabled:opacity-70 transition-opacity">
            {/* Row 1 */}
      ...
      <div className="fixed bottom-4 sm:bottom-6 left-1/2 -translate-x-1/2 z-40 w-[90%] sm:w-auto">
        ...
        >
          {isExporting ? <Loader2 className="animate-spin w-6 h-6" /> : <CloudUpload className="w-6 h-6" />}
...
>>>>>>> f4312b2253d9fbbd0a0ec05805698aa46765103e
